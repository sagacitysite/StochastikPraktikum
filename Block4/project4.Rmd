...
fnSimulatePoisson <- function(lambdaT, nSimulations = 10000) {
  # Get random poisson distributed values for nSimulations days
  vSimulated <- rpois(length(lambdaT)*nSimulations, lambdaT)
  # Split data into matrix of form: mSplitted[day, quarter-hour]
  # where day goes from 1 to nSimulations and quater-hour from 1 to 96
  mSplitted <- t(matrix(simulated, nrow=96, ncol=nSimulations))
  
  return(mSplitted)
}

splitted <- fnSimulatePoisson(lambdaT)

# task 1
meanCarsPerDay <- mean(rowSums(splitted))

# task 2
thousandsCar <- NULL
for(i in 1:nSimulations) {
  cumsumDay <- cumsum(splitted[i,])
  # You leave "floor" here
  # It's not clear if floor should ne applies only for mean, or also here.
  # Howeever, in the end the results seems to be the same
...
meanCarsAtNightQuarter <- mean(as.vector(splitted[,7]))
