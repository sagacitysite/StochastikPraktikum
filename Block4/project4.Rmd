...
Methode 1 benötigte `r round(as.double(timeMethod1, units = "secs"), 1)` Sekunden, wohingegen Methode 2 `r round(as.double(timeMethod2, units = "secs"), 1)` Sekunden für die Approximation benötigte. Die Ergebnisse für Blase, Gehirn und Herz sind in folgender Tabelle zusammengefasst:
...
randValsX <- matrix(rnorm(walks*steps,0,1),nrow=walks,ncol=steps)
randValsY <- matrix(rnorm(walks*steps,0,1),nrow=walks,ncol=steps)
randValsZ <- matrix(rnorm(walks*steps,0,1),nrow=walks,ncol=steps)
...
Als Trennung schonmal die Chunks eingefügt
```{r}
#Lalalalala
...
Jetzt nähern wir das Resultat der Black-Scholes Formel mit Hilfe einer MC-Simulation, und mit dem in der Vorlesung vorgestellten Euler-Schema:
