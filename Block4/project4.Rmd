...
Wie aus dem Theorem von Polya ersichtlich, findet der Mensch in endlicher Laufzeit mit deutlich höherer Wahrscheinlichkeit nach Hause als der Vogel, im Falle $n \to \infty$ konvergiert die "Heimkehr-Wahrscheinlichkeit" sogar gegen 1.
...
Für eine Überprüfung der Grenzwerte, werden die drei Fälle simuliert. Die Plots in der Abbildung zeigen die Fälle $\mu - \frac{\sigma^2}{2} = -0,0025$ (links), $\mu - \frac{\sigma^2}{2} = 0$ (mitte) und $\mu - \frac{\sigma^2}{2} = 0,0025$ (rechts).

```{r Plot Geometric Brownian Motion for different parameters}
...
Die schwarze Linie markiert den Mittelwert der `R nSteps` Durchgänge, angegeben sind jeweils das gewählte $\mu$ und $\sigma^2$.
