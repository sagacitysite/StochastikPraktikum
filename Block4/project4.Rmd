...
# TOO: add MonteCarlo wrapper function for the simulation
...
# Plotting
sf <- data.frame(X=10, Y=10)
sff <- data.frame(X=0, Y=0)
p1 <- ggplot(humanWalk, aes(x=X1, y=X2, color=X3)) +
    geom_path() +
    geom_point(data=sf,aes(X,Y),colour='red',size=4) +
    geom_point(data=sff,aes(X,Y),colour='green',size=4)
p1

# Plotting 2
birdWalk <- fnRandomWalk(3, c(10,10,10), 1000, 3)
p2 <- ggplot(birdWalk, aes(x=X1, y=X2, z=X3, color=X4)) +
    geom_path()
p2

```

In the plots we see an example of each random walk.
bla
bla
bla
...
Now we move on to simulating the random walk in d=2 and d=3, and its state recurrence, via a Monte Carlo simulation
...
fnRandomWalkMC <- function(nSim, nSamples, R) {
  ...
  #
  # ongoing dev:
  # for each number in nSim, simulate nSamples random walks and store their exit status
  # todo: add % of "got home" walks to dfRand as column, use X1 column for values of nSim
  dfRand <- data.frame(...)
  for (i in nSim) {
    dfB <- NULL
    dfH <- NULL
    for (j in nSamples) {
      dfB[j] <- dim(fnRandomWalk(2, startpos[1:2], i, R))[[1]]
      dfH[j] <- dim(fnRandomWalk(3, startpos[1:3], i, R))[[1]]
    }
    dfRand[something] <- c(i, sum(dfB<i)/nSamples, sum(dfH<i)/nSamples)
...
#startpos <- c(10,10,10)
#nSim <- c(10, 50, 100, 200, 500, 1000, 2000, 4000, 6000, 10000)
#dfMC <- fnRandomWalkMC(nSim, 30, 3)
