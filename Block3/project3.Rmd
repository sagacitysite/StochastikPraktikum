...
```{r Indicators, fig.cap = "Wordclouds for SPAM (left) and HAM (right) indicators."}
par(mfrow = c(1,2))
...
$$
\E[\| \hat\rho - \rho \|^2] = \E[(\hat\rho - \rho)^T (\hat\rho - \rho)] = \left( \E[\hat\rho] - \rho \right)^2 + \Var(\hat\rho) = \text{Bias}(\hat\rho) + \text{Varianz}(\hat\rho)
$$

Für den Kleinste-Quadrate-Schätzer $\hat\beta = (X^T X)^{-1} X^T Y$ ergibt sich damit:

$$
\E[\| \hat\beta - \beta \|^2] = (\beta - \beta)^2 + \Var(\hat\beta) = \sigma^2 (X^T X)^{-1}
$$
...
$$
\E[\| \hat\beta^{RR} - \beta \|^2] = \left[ (X^T X + \lambda I_p)^{-1} X^T X \beta - \beta \right]^2 + \sigma^2 (X^T X + \lambda I_p)^{-1} X^T X (X^T X + \lambda I_p)^{-1}
$$
