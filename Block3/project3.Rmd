...
Diskussion von "Additional Features"
...
Für die Überprüfung dieser Eigenschaft gibt es verschiedene Verfahren, von denen die k-fold Cross-Validation eines der bekanntesten darstellt.

Bei diesem Verfahren wird der gesamte Datensatz in k gleichgroße Partitionen, sogenannte *folds* geteilt. Im Anschluss wird k-mal ein Classifier trainiert, und für jeden Durchgang i=1..k wird jeweils die i-te fold als Test-/Validierungsset zurückgehalten.
Gegenüber dem einfachen Verfahren hat die k-fold Cross-Validation vor allem zwei Vorteile:
1. Es werden alle Daten zum Trainieren verwendet.
2. Man erhält k verschiedene Classifier anstatt einem einzigen, und kann so ebenfalls Aussagen über das Modell (average fit, Generalisierbarkeit) treffen.
...
fnCrossValidation <- function(nFolds, X, Y) {
  
  
  nDataPoints <- dim(XData)[1]
  
  splitInd <- sample(seq(nDataPoints), round(nDataPoints/nFolds))
  XDataTrain <- XData[-splitInd, ]
  YDataTrain <- YData[-splitInd, ]
  XDataTest <- XData[splitInd, ]
  YDataTest <- YData[splitInd, ]
  
  nErrors <- vector('double', nFolds)
  
  for (k in nFolds) {
    
    NBClass2 <- fnNBTrain(XDataTrain, YDataTrain[[1]])
    # Predict test labels
    prediction <- fnNBPredict(XDataTest, NBClass2)
    # Number of errors
    nErrorRate[k] <- sum(YDataTest[[1]] != prediction)/sum(YDataTest[[1]])
  }
  
  return(nErrorRate)
}

nCrossValErrors <- fnCrossValidation(6, XData, YData)
...
#1: Combine data, 2. split data, 3. train k times, 4. get average accuracy, 5. report 

```

Auswertung von `r nCrossValErrors`
...
Deutlich wird das im orthogonalen Fall, wenn $X^T X = I_p$. Dann gilt für $\hat\beta = X^T Y$ und für $\hat\beta^{RR} = \frac{X^T Y}{1 + \lambda} = \frac{\hat\beta}{1 + \lambda}$. Die geschätzten Kleinste-Quadrate Parameter $\hat\beta$ werden für $\lambda > 0$ also immer kleiner sein als die der Ridge-Regression $\hat\beta^{RR}$.
