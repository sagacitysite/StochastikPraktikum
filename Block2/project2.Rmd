...
fnMNearestNeighbors <- function(x, X, K, m) {
  ...
  for(xi in x) {
    dist <- abs(X - xi)
    h <- c(h, abs(sort(X[which(dist %in% sort(dist)[1:m])])[m]))
  }
...
plot(x, fnMNearestNeighbors(x, X, fnGaussianKernel, 5), type = "l", main = "title", xlab = "x", ylab = "Density")
lines(density(X, bw = "ucv"))

m <- c(2,5,10,25,50)

fnKernelPlot(x, X, fnGaussianKernel, vm = m, fnEstimation = fnMNearestNeighbors,
             title = "Nearest Neighbor Geaussian Kernel Density Estimation",
             trueDensity = dnorm, ylim = c(0, 0.6))
